...
import src.inner_product.single_input_fe.elgamal_ip.edfe
import numpy as np
import unittest
...
class TestElGamalInnerProduct(unittest.TestCase):
    ...
    def test_fin_result(self):
        ...
        # 分配公钥私钥
        fe = src.inner_product.single_input_fe.elgamal_ip.edfe  # 函数y
        # pk, sk, N = fe.set_up(1024, 5) # 调用edfe setup 算法，秘钥是1024位，5对密钥
        pk, sk, N = fe.set_up(1024, 7) # 调用edfe setup 算法，秘钥是1024位，7对密钥,是所有的秘钥生成，后面分配
        #TP
        ...
        pk3,sk3 = pk[6],sk[6]   # decryptor

        # y = [1, 1, 1]   # 聚合权重
        # x = [1, 2, 3]   # 加密者的明文
        y = [1, 1, 1, 1, 1]   # 聚合权重
        x = [1, 2, 3, 4, 4]   # 加密者的明文，X长度和客户端个数一样
        ctr=0  # 计数器，后面每次加1
        aux='edfe' # 改名字edfe
        ...
        result = fe.Decrypt(C_x,y,N)  # 内积
        print("EDFE result is:", result)  # 改名字
