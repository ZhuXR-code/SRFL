...
def KeyDerive(master_secret_key: List[ElGamalKey], master_public_key2:List[ElGamalKey], master_secret_key2: List[ElGamalKey], ctr: int, y: List[int], aux: str) -> integer:
    ...
    # print( ">>>>> KeyDerive   len(y)",len(y))
    # print( ">>>>> KeyDerive   len(master_secret_key2)",len(master_secret_key2))
    for i in range(len(y)):
        # print("y[",i,"]:",y[i])
        # print(master_secret_key2[i])
...
def Encrypt(
            master_public_key: dict,
            master_secret_key2: dict,
            master_public_key3: dict,
            ctr: int,
            X: List[int],
            aux: str,
            N: int
    ...
    # print('len(X)',len(X))
    # print('len(master_secret_key2)',len(master_secret_key2))
...
def Decrypt(C: List[integer], y: List[int], N: integer) -> integer:

    """
    解密函数，用于解密密文并恢复原始消息。

    参数:
    - master_public_key: 主公钥，包含ElGamal密钥列表。
    - skf: 秘密密钥因子。
    - master_secret_key3: 主密钥的第三部分，包含ElGamal密钥列表。
    - C: 密文列表，每个密文包含两个部分。
    - y: 权重列表，用于计算密文的乘积。
    - N: 模数，用于计算模平方。
    ...
    '''
    # 代码解释
    这段代码用于计算密文的乘积并取模。具体步骤如下：
    1. 初始化 `c32` 和 `c42` 为1。
    2. 遍历密文列表 `C`，对于每个密文 `C[i]`：
       - 计算 `C[i]['0']` 的 `y[i]` 次幂，并更新 `c32`。
       - 计算 `C[i]['1']` 的 `y[i]` 次幂，并更新 `c42`。
    3. 对 `c32` 取模 `N_squared`。
    '''
